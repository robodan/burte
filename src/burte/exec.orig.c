...
/*----------------------Freeze */
/* suspend schedualing and interupts */
int r_freeze()
{
    burte_state = FROZEN;
}
...
/*----------------------Thaw */
/* resume shedualing and interupts */
int r_thaw()
{
    burte_state = ACTIVE;
}
...
/*  the value of the mailbox is returned by the schedualer */
  ...
  short proc;
{
	...
	if (! run_que[i]) {
	    run_que[i] = pr;
	    return;
	} else if (pr->prty > run_que[i]->prty) {
	    for (j=i; (run_que[j]) && (j < MAX_PROC-1); j++) { /* shift down */
