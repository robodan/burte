...
|	d1	scratch
	...
	.globl	new_proc
clear_trap:
	addql	#6,sp		|remove trap call from stack
new_proc:
...
j_a_e7:		|restart previous process
...
_r_save:	| executive save
	...
	blts	j_a_e2			|if d1 <0 or > MAX_PROC then this
...
j_a_e2:		|invalid id in _current_proc
	movw	#0x2700,sr
	movl	#12,sp@-	|error 1
...
_user_save:	|user save
	movw	_current_proc,d1	|array subscript
	blts	j_a_e3			|if d1 <=0 or > MAX_PR then this
	cmpw	#MAX_PR,d1		| is not a valid ID
	bges	j_a_e3
...
j_a_e3:		|invalid id in _current_proc
	movl	#1,sp@-		|error 2
	jsr	_burte_error	|die with a system error
	rts
...
_bad_id:
	...
	blts	j_a_e4			|if d1 <0 or >= MAX_PR then this
