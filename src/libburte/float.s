...
|
	.globl	_fix
	.globl	__fixunsdfsi		|This is wrong? unsigned
	.globl	__fixdfsi
	.proc
	.text
__fixuns_truncdfsi:
...
|fcmp returns -1 if a < b; 1 if a > b; 0 a == b.
|
	.globl	_fsub
	.globl	__subdf3
	.globl	_fadd
...
_fcmp:
	...
	tstl	d0			|set condition codes
	blt	1$			|check < 0
	bgt	2$			|check > 0
	bra	3$			| == 0
1$:	moveq	#-1,d0			| put -1 into D0
	bra	3$
2$:	moveq	#1, d0			| put 1 into D0
3$:	unlk	a6
	rts
_afadd:
	link	a6,#0
...
add:
	moveml	#0x3000,sp@-		|save d2,d3
	movw	a0@(EXPT),d0		|load
	movw	a1@(EXPT),d1		|  exponents

	tstl	a0@(MANH)		| Zero becomes a special
	bne	add1			|   case since internal
	tstl	a0@(MANL)
	bne	add1
	movl	a1, a0			|   format contains
	bra	qadd			|   unbiased exponents
add1:					| ????
	tstl	a1@(MANH)
	bne	add2
	tstl	a1@(MANL)
	beq	qadd
add2:
	addw	#128, d0		|bias the
	addw	#128, d1		|  exponents
	subw	d1,d0			|compare exp's of a and b
	cmpw	#58, d0		|is the difference too big?
	blt	sadd			|no, got to do the addition (slow add)
	tstw	d0			|check if a0 points to the bigger
	bpl	qadd			|yes
	movl	a1, a0			|no, force it
	bra	qadd
sadd:
	tstw	d0			|check the sign of (a_expt - b_expt)
	bmi	1$			|branch if a_expt < b_expt
...
ok:	movl	a1@,d1			|d1 = divisor low
	...
	moveq	#56,d6			|setup shift count
	bra	2$

1$:	addql	#1,d5			|add to
	bcc	2$			|  quotient
	addql	#1,d4
	subqw	#1,d6			|exit computation when
	beq	dive			|  loop count = 0
2$:	subl	d1,d3			|subtract divisor from
	bcc	3$			|
	subql	#1,d2			|
3$:	subl	d0,d2			|  remainder
	bpl	1$

	addl	d1,d3			|add divisor back to
	addxl	d0,d2			|  remainder
	asll	#1,d5			|shift 0 into quotient
	roxll	#1,d4			|  accumulator registers
	asll	#1,d3			|continue shift into
